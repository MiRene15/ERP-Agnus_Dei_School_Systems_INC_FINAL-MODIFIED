...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-[#E8EAF6]">Consultation Logs</h2>
        <p class="text-gray-600 dark:text-[#C1C4DC] mt-1">Record of all clinic visits and consultations.</p>
    </div>
...
<div x-data="ajaxTable('{{ route('nurse.logs') }}', { search: '{{ request('search') }}', incident_type: '{{ request('incident_type') }}', sickness: '{{ request('sickness') }}', month: '{{ request('month') }}', grade_level: '{{ request('grade_level') }}', date_from: '{{ request('date_from') }}', date_to: '{{ request('date_to') }}' })">
    <div class="mb-4 flex gap-2 flex-wrap items-center">
        <form method="GET" class="flex gap-2 flex-1 flex-wrap" @submit.prevent="reload()">
            <input type="text" name="search" x-model="filters.search" @input.debounce.300ms="reload()" placeholder="Search by student name..." class="flex-1 min-w-[200px] rounded-lg border border-gray-300 dark:border-[#3B4172] dark:bg-[#23274C] dark:text-[#E8EAF6] px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            ...
            <select name="month" x-model="filters.month" @change="reload()" class="rounded-lg border border-gray-300 dark:border-[#3B4172] dark:bg-[#23274C] dark:text-[#E8EAF6] px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
